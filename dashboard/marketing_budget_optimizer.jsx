...
function BriefingTab() {
  const sections = [
    ...
    },
    {
      ...
      content: `We constructed a Bayesian Marketing Mix Model using PyMC-Marketing, fitted on 104 weeks (Jan 2024 – Dec 2025) of weekly channel-level spend and revenue data across six paid channels plus organic and direct traffic. The model uses two learned transformations: (1) LogisticSaturation (1 − e^(−λ·x)) where λ is learned per channel via MCMC — capturing diminishing returns without hardcoding saturation constants, (2) GeometricAdstock with learned decay parameter α per channel (prior: Beta(1,3)) — modeling carryover effects with data-driven decay rates. All parameters are estimated jointly through Bayesian MCMC sampling (4 chains × 1000 draws), producing full posterior distributions with 90% credible intervals on every metric. The baseline is decomposed from the model's intercept, Fourier seasonality, and control variable coefficients. This follows the industry-standard approach used by Meta Robyn, Google Meridian, and PyMC-Marketing.`,
    },
    {
      label: "MODEL SPECIFICATION: WHY BAYESIAN MMM WITH PyMC-MARKETING",
      content: `We evaluated four approaches and selected Bayesian MMM: (1) Plain linear regression — fast but assumes constant marginal returns, contradicting marketing theory. (2) Ridge / regularized regression (Meta Robyn) — handles multicollinearity but provides only point estimates without uncertainty. (3) Log-linear with hardcoded transforms (our previous approach) — OLS on pre-transformed features, interpretable but requires manually setting saturation and adstock parameters. (4) Full Bayesian MMM via PyMC-Marketing (our current choice) — learns all parameters (saturation λ, adstock decay α, channel betas β) jointly from data through MCMC sampling, producing posterior distributions with credible intervals on every metric. We chose Bayesian because: uncertainty quantification is essential for credible budget recommendations (knowing "Email ROI = 5.2x ± 1.4x" vs just "5.2x" changes decisions); prior regularization naturally constrains channel effects to be positive without ad-hoc fixes; and hyperparameters are learned from data rather than hardcoded or grid-searched, making the model genuinely data-driven.`,
    },
    {
      label: "BAYESIAN PARAMETERS & UNCERTAINTY QUANTIFICATION",
      content: `All parameters are learned from data via MCMC with informative priors: (a) Saturation rate (λ) — controls the curvature of diminishing returns; learned per channel with Gamma(3,1) prior, producing posterior mean and 90% credible interval. (b) Adstock decay (α) — geometric decay capturing carryover effects; learned per channel with Beta(1,3) prior. (c) Channel coefficient (β) — media effectiveness at scale; learned with HalfNormal prior scaled by channel spend share (higher-spend channels get wider priors). Every parameter comes with a full posterior distribution, enabling statements like "TV/OTT decay = 0.35 [0.21, 0.48] with 90% confidence." Implementation uses PyMC-Marketing (Python), the most widely-adopted open-source Bayesian MMM framework, built on PyMC and ArviZ. Alternatives include Meta's Robyn (ridge + Nevergrad — faster but no uncertainty) and Google's Meridian (Bayesian + JAX — optimized for geo-level data).`,
    },
  ];
...
function InsightsTab() {
  const findings = [
    ...
    { title: "TV/OTT: Low direct ROAS (1.5x), but true value is likely understated",
      ...
      action: "Do NOT cut TV/OTT based solely on modeled ROAS. Instead: (1) Run a geo-holdout test — dark TV in 2-3 DMAs for 8 weeks and measure total revenue (not just TV-attributed). (2) Analyze the correlation between TV spend and branded search volume with 2-4 week lag. (3) Use the Bayesian posterior to set informative priors on TV's long-tail effect — the learned adstock decay and its credible interval provide a principled basis. Recommended floor: $38.5K/week until incrementality is validated.",
      impact: "high" },
    ...
    <div>
      ...
      <Card style={{ background: DS.surfaceAlt }}>
        ...
        <p style={{ margin: 0, fontSize: 12.5, color: DS.text.secondary, lineHeight: 1.8 }}>
          Bayesian Marketing Mix Model via PyMC-Marketing. LogisticSaturation (1 − e<sup style={{ fontSize: 9 }}>−λ·x</sup>) with learned λ per channel
          and GeometricAdstock with learned decay α per channel. All parameters estimated via MCMC (4 chains × 1000 posterior draws).
          Priors: adstock α ~ Beta(1,3), saturation λ ~ Gamma(3,1), channel β ~ HalfNormal(σ=spend_share).
          Baseline decomposition: intercept + trend + Fourier seasonality + holiday controls — a model output, not an assumption.
          Every metric includes 90% credible intervals from the posterior distribution.
          Model R² = {META.r2}. Optimization: marginal ROI equalization with uncertainty quantification (P(lift{'>'}0) reported).
          Built in Python (PyMC-Marketing + ArviZ). Open-source alternatives: Meta Robyn, Google Meridian.
        </p>
      </Card>
    </div>
  );
